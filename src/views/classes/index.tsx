...
const TIERS = ["beginner", "unique", "intermediate", "advanced", "master"];
...
export class ClassesView extends React.Component<any, ClassesViewState> {
    ...
    private matchToRequirement(matching: string[], requirements: IRequirement[]): boolean {
        ...
        return (
            requirements &&
            requirements.reduce((previous, current) => previous || matching.includes(current.name), matched)
        );
    }
    ...
    private filterResults() {
        ...
        return classes.filter(job => {
            const matchSkills = skills.length > 0 ? this.matchToRequirement(skills, job.Requirements) : true;
            const matchTiers = tiers.length > 0 ? tiers.includes(job.Tier) : true;

            return matchSkills && matchTiers;
        });
    }
            ...
            <Container fluid>
                <Segment.Group raised>
                    ...
                    <Segment basic textAlign="center" horizontal={IS_MOBILE}>
                        ...
                        {Skills.map(skill => (
                            <Button
                                onClick={() => this.updateSkills(skill)}
                                positive={this.state.skills.includes(skill)}
                                compact
                            >
                    ...
                    <Segment basic horizontal={IS_MOBILE} textAlign="center">
                        ...
                        {TIERS.map(tier => (
                            <Button
                                onClick={() => this.updateTiers(tier)}
                                positive={this.state.tiers.includes(tier)}
                                compact
                            >
