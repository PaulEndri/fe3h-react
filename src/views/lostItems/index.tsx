...
export const LostItemsView = () => {
    ...
    if (characterSearchTerm.length >= 3) {
        localCharacters = localCharacters.filter((c) => c.character.name.toLowerCase().includes(characterSearchTerm.toLowerCase()));
    }

    if (itemSearchTerm.length >= 3) {
        localCharacters = localCharacters.filter((c) => c.character.lostItems.some((item) => item.toLowerCase().includes(itemSearchTerm.toLowerCase())));
    }
                    ...
                    <Card key={index} raised >
                        ...
                        <Responsive minWidth={Responsive.onlyMobile.maxWidth}>
                            <Card.Content>
                                ...
                                {itemSearchTerm && character.character.lostItems.some((item) => item.toLowerCase().includes(itemSearchTerm)) && (
                                    <Card.Meta>
                                        <Label tag color="green">Match Found</Label>
                                    </Card.Meta>
                                )}
